...
void set_retrans_timer(struct tcp_sock *tsk)
{
...
void insert_snd_buf(struct tcp_sock *tsk, char *packet, int len, u32 seq_end)
{
	sent_packet_t *p = (sent_packet_t *)malloc(sizeof(sent_packet_t));
	char *packet_cpy = (char *)malloc(len);
	assert(packet_cpy != NULL);
	memcpy(packet_cpy, packet, len);
	p->packet = packet_cpy;
	p->len = len;
	p->seq_end = seq_end;
	list_add_tail(&p->list, &tsk->snd_buf);
	set_retrans_timer(tsk);
}
...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len)
{
	...
	insert_snd_buf(tsk, packet, len, tsk->snd_nxt);
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	{
		tsk->snd_nxt += 1;
		insert_snd_buf(tsk, packet, pkt_size, tsk->snd_nxt);
	}
