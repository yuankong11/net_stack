...
void handle_packet(iface_info_t *iface, char *packet, int len)
{
	...
	switch (ntohs(eh->ether_type)) {
		...
		case ETH_P_ARP:
			...
			break;
		default:
			log(ERROR, "Unknown packet type 0x%04hx, shost: "ETHER_STRING", dhost: "ETHER_STRING", ingore it.", ntohs(eh->ether_type), ETHER_FMT(eh->ether_shost), ETHER_FMT(eh->ether_dhost));
			break;
	}
}
